...
**关键决策**（用户确认）：@提及触发 · 消息落 issues 复用现有流水线 · 先单聊闭环再群聊 · **交流内容不沉淀进知识库**（靠会话历史临时查）。

### 12.1 知识库模型（M7 定稿，2026-08-15）

**双层知识库**：

| 层 | 装什么 | 谁写 | 谁读 |
|---|---|---|---|
| `team`（共有） | 项目介绍、注意事项、规范 | 老板（人类） | 所有 agent |
| `agent_<id>`（成员私有） | 老板发的文档、agent 写过的代码 | 该 agent + 老板 | 只有本人 |

- `memory` 表：`scope`（team / agent_<id>）+ `kind`（文档 / 代码）+ `content` + `agent_id`
- **交流内容不沉淀**：消息是过程（活水），靠会话历史临时查（messages 表，按会话隔离）；知识库是沉淀（文档 + 代码）
- 执行时注入：team 知识 + 本人知识库 + 相关会话历史（最近 N 条）
- 现仓库根目录 CLAUDE.md 即"team 知识库"的文件原型，M7 搬进数据库结构化（可分类、可更新、可 SQL 查询）

**已落地**：M6 飞书式前端、M7 双层知识库（`memory` 表 + `internal/memory` + `/api/memory`）、M8 触发语义（单聊默认派活 / 群聊 @ + 成员校验 + 多 @）。
